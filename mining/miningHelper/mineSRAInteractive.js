...
// 'accessionsWithPublications' is a dictionary with SRA accession (SRX numbers) keys and publication PubMed id values, such that the SRX number is associated with that PubMed id.
const mineSRAInteractive = (multipleSrpPublications, publications, sras) => {
  for (let pmId in multipleSrpPublications) {
      ...
      if (d.indexOf("<title>Error - SRA - NCBI</title>") === -1 && d.indexOf("<title>No items found - SRA - NCBI</title>") === -1) {  // check if SRA datasets exist for this publication
        ...
        while (moreSRAs) {
          ...
          if(sra_detail != "failed"){
            ...
            let srp = str;
            if (publications[multipleSrpPublications[accession]].sra_accession.indexOf(str) === -1) {  // there are multiple accessions (SRX numbers) per SRP number, only save the unique SRPs
              publications[multipleSrpPublications[accession]].sra_accession.push(str);                //  such that an SRP number is related to an SRA data set
            }
